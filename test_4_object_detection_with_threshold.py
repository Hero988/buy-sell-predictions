...
def save_candlestick_charts(data, sequence_length, future_period, output_dir, symbol, threshold):
    """
    ...
    for i in range(len(data) - sequence_length - future_period):
        ...
        # Get the last close from the future data
        future_last_close = future_data['close'].iloc[-1]

        # Calculate the percentage change
        percentage_change = ((future_last_close - last_close) / last_close) * 100

        # Classification based on future candlestick prices and threshold
        if (future_data['close'] > last_close).all() and (future_data['high'] > last_close).all() and (future_data['low'] > last_close).all() and (percentage_change > threshold):
            class_folder = "buy"
            class_label = 0  # YOLO class ID
        elif (future_data['close'] < last_close).all() and (future_data['high'] < last_close).all() and (future_data['low'] < last_close).all() and (percentage_change < -threshold):
            class_folder = "sell"
...
if __name__ == "__main__":
    ...
    timeframe = "1h"
    output_dir = "candlestick_charts_with_threshold"
    sequence_length = 12
    future_period = 6
    threshold = 0.5
    ...
    for symbol in symbols:
        ...
        if raw_data is not None:
            ...
            save_candlestick_charts(enhanced_data, sequence_length, future_period, output_dir, symbol, threshold)